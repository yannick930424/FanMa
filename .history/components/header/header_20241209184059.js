...
navigationItems: [
    ...
    { key: 'contact', translationKey: 'nav_contact', path: '/contact' }
        ...
        getHomeUrl() {
            return `/lang/${this.$store.language.current}/`;
        },

        getNavUrl(item) {
            const basePath = `/lang/${this.$store.language.current}`;
        ...
        isCurrentPage(key) {
            ...
            // Ajuster la vérification pour inclure /lang/{fr|en}/ dans le chemin
            const fullPath = `/lang/${this.$store.language.current}${itemPath}`;
